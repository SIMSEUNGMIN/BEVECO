...
public class SearchTour extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        list_tour.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                String url = "https://search.naver.com/search.naver?sm=top_hty&fbm=1&ie=utf8&query=";
                String title = tourAdapter.getTitle(position);
    ...
    private void clickSearchButton() {
        ...
        String[] strData = splitSpace(searchTour.getText().toString());
        String[] address;
        String tour;

        for(int i = 0; i < rowTotal - 1; i++){

            address = splitSpace(tourAdapter.getAddress(i));
            tour = tourAdapter.getTitle(i);

            switch (address.length){ //주소가 일치하거나 상호명이 일치.
                case 3 :
                    if(strData.length == 0){
                        break;
                    }
                        ...
                        if(address[2].contains(strData[0])
                                || tour.contains(strData[0])){
                            searchTourAdapter.addItem(tourAdapter.getTitle(i), tourAdapter.getAddress(i), tourAdapter.getTel(i));
                        }
                    }
                    else if(strData.length == 2){
                        if(address[2].contains(strData[0]) || address[2].contains(strData[1])
                                || tour.contains(strData[0])){
                            searchTourAdapter.addItem(tourAdapter.getTitle(i), tourAdapter.getAddress(i), tourAdapter.getTel(i));
                        }
                    }
                    else if(strData.length >= 3){
                        if(address[2].contains(strData[0]) || address[2].contains(strData[1]) || address[2].contains(strData[2])
                                || tour.contains(strData[0])){
                            searchTourAdapter.addItem(tourAdapter.getTitle(i), tourAdapter.getAddress(i), tourAdapter.getTel(i));
                        }
                    }
                ...
                case 4 :
                    if(strData.length == 0){
                        ...
                        if((address[2].contains(strData[0]) && address[3].contains(strData[1]))
                                || (address[2].contains(strData[1]) && address[3].contains(strData[0]))
                                || tour.contains(strData[0])){
                            searchTourAdapter.addItem(tourAdapter.getTitle(i), tourAdapter.getAddress(i), tourAdapter.getTel(i));
                        }
                    }
                ...
                default:
                    if(strData.length == 0){
                        ...
                        if(address[2].contains(strData[0]) || address[3].contains(strData[0]) || address[4].contains(strData[0])
                                || tour.contains(strData[0])){
                            searchTourAdapter.addItem(tourAdapter.getTitle(i), tourAdapter.getAddress(i), tourAdapter.getTel(i));
                        }
                    }
                        ...
                        if((address[2].contains(strData[0]) && address[3].contains(strData[1]) && address[4].contains(strData[2]))
                                || tour.contains(strData[0])){
                            searchTourAdapter.addItem(tourAdapter.getTitle(i), tourAdapter.getAddress(i), tourAdapter.getTel(i));
                        }
                    }
